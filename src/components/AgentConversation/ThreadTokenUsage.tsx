...
function ThreadTokenUsage({chatTokens=0}:Props) {
    ...
    return (
        <Flex alignItems="center">
            <Tooltip label="Number of tokens processed in this chat, including custom instructions, linked data sources and API interfaces." fontSize="sm">
                <span>
                    <InfoOutlineIcon cursor="help" boxSize="14px" mb="1" color="primary.50" />
                </span>
            </Tooltip>
            <Text fontSize='14px' display="inline-block" ml="2">
                Chat Tokens: {new Intl.NumberFormat().format(chatTokens)}
            </Text>
            <Box height="16px" width="1px" bg="gray.200" mx="3"></Box>
            <Text fontSize="14px">
                Remaining Tokens:{" ∞"}
                {/*new Intl.NumberFormat().format(remainingTokens)*/}
            </Text>
            <Tooltip label="Infinity tokens for our beta testers!" fontSize="sm">
                <span>
